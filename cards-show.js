class CardShow {
    constructor(element) {
        ...
        document.querySelector('.card__form').addEventListener('click',this.cardsClick);

        this.buttonBeginNewGameHandler = this.buttonBeginNewGameHandler.bind(this);
        document.querySelector('.btn__new_game').addEventListener('click', this.buttonBeginNewGameHandler);
    ...
    renderItem(item) {
        this.element.lastChild.appendChild(templateEngine(this.cardList.map(CardShow.templateItem)));
        
        const shirtBegin = () => {
            this.element.lastChild.innerHTML = '';
            this.element.lastChild.appendChild(templateEngine(this.cardList.map(CardShow.templateItemShirt)));
